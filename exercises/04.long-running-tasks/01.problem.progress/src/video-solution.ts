...
export async function createWrappedVideo({
	...
	year: number
	mockTime?: number
	// Optional callback for progress reporting during video creation
	// Progress values range from 0 (start) to 1 (complete)
	onProgress?: (progress: number) => void
}) {
	const videoFilename = `wrapped-${year}.mp4`

	// Mock mode for testing - simulates video creation with progress updates
	if (mockTime && mockTime > 0) {
		const step = mockTime / 10
		for (let i = 0; i < mockTime; i += step) {
			const progress = i / mockTime
			if (progress >= 1) break
			// Report progress during mock simulation
			onProgress?.(progress)
	...
	// Spawn ffmpeg process to create the actual video
	...
	const ffmpegPromise = new Promise((resolve, reject) => {
		...
		// Monitor ffmpeg stderr output to track encoding progress
		if (ffmpeg.stderr) {
			ffmpeg.stderr.on('data', (data) => {
				const str = data.toString()
				// Parse ffmpeg's time output to calculate progress
				// Format: time=00:00:05.23 (hours:minutes:seconds.fraction)
				...
				if (timeMatch) {
					...
					// Calculate progress as fraction of total duration
	...
	// Notify any subscribers that a new video is available
	notifySubscribers()
