...
export async function initializeTools(agent: EpicMeMCP) {
	...
	agent.server.registerTool(
		...
		async (
			...
			// The second parameter provides access to MCP server utilities
			// sendNotification allows us to send progress updates to the client
			// _meta contains metadata including the progressToken for tracking progress
			const entries = await agent.db.getEntries()
			...
			const videoUri = await createWrappedVideo({
				...
				year,
				mockTime,
				// Progress callback that sends notifications to the client
				// This allows the client to show progress bars or status updates
				onProgress: (progress: number) => {
					const { progressToken } = _meta ?? {}
					// Only send progress updates if the client provided a progress token
					// This indicates the client is interested in receiving progress notifications
					if (!progressToken) return
					
					// Send progress notification using the MCP protocol
					// The client can use this to update UI progress indicators
					void sendNotification({
						method: 'notifications/progress',
						params: {
							progressToken, // Token to identify this specific request
							progress,     // Current progress (0-1)
							total: 1,     // Total amount (1 means progress is a fraction)
							message: 'Creating video...', // User-friendly status message
						},
					})
				},
			})
