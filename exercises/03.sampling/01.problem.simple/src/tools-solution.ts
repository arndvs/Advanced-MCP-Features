...
// Import our sampling function to automatically suggest tags when entries are created
import { suggestTagsSampling } from './sampling.ts'
import { createWrappedVideo } from './video.ts'

export async function initializeTools(agent: EpicMeMCP) {
	// SAMPLING: The create_entry tool demonstrates how to borrow the user's LLM
	// When a new entry is created, we automatically request tag suggestions
	// The sampling request asks the client (like VS Code) to use the user's LLM
	// This allows MCP servers to provide AI-powered features without their own LLM
	agent.server.registerTool(
		...
		async (entry) => {
			...
			// Trigger automatic tag suggestion via sampling (borrowing user's LLM)
			// Using 'void' instead of 'await' makes this a fire-and-forget operation
			// The user gets their response immediately, sampling happens in background
			// This prevents blocking the create_entry tool response if user denies sampling
