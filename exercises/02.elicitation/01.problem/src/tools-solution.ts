// SOLUTION: Elicitation Exercise - User Confirmation Implementation
// This solution demonstrates the complete implementation of elicitation
// for getting user confirmation before destructive operations.
// Key concepts:
// 1. Check client capabilities before attempting elicitation
// 2. Use elicitInput to request user confirmation during tool execution
// 3. Handle different response types (accept/decline/cancel)
// 4. Graceful fallback when elicitation isn't supported
...
export async function initializeTools(agent: EpicMeMCP) {
	...
	agent.server.registerTool(
		...
		async ({ id }) => {
			...
			// SOLUTION: Implement elicitation for user confirmation
			// First check if the client supports elicitation capability
			const capabilities = agent.server.server.getClientCapabilities()
			if (capabilities?.elicitation) {
				// Elicit user confirmation - server requests input from client
				...
				if (!confirmed) {
					// User declined or cancelled - return early without deleting
			...
			// Proceed with deletion after confirmation or if elicitation not supported
